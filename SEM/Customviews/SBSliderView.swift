...
func createSlider(withImages images: [Any], withAutoScroll isAutoScrollEnabled: Bool, in parentView: UIView) {
    ...
    pageIndicator.tintColor = UIColor.grayMainColor
    pageIndicator.pageIndicatorTintColor = UIColor.grayMainColor
    pageIndicator.currentPageIndicatorTintColor = UIColor.yellowMainColor
    
    sliderMainScroller.contentSize = CGSize(width:CGFloat(UIScreen.main.bounds.size.width*3*(CGFloat(imagesArray.count))), height:CGFloat(parentView.frame.size.height))
    var mainCount: Int = 0
...
//            let imageName:String = "\(imagesArray[i])"
            imageV.image = imagesArray[i] as? UIImage
    ...
    delegate?.sbslider(self , didTapOn: (targetView?.image)!, andParentView: targetView!, andWithTag: tag)
